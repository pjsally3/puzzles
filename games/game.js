(() => {
  ...
  function computePositions(seqChars, W, H) {
    ...
    const rowYs = (rows === 2)
      ? [centerY - 70, centerY + 70]
      : [centerY - 90, centerY, centerY + 90];
  ...
  function computeEdgeParamsWithLanes(edgesArr, centerY) {
    ...
    for (const e of edgesArr) {
      ...
      const ra = rcMap.get(a), rb = rcMap.get(b);
  ...
  function drawCurvedArrow(pa, pb, offsetY, color, width=3){
    ...
    for (let i=1;i<=30;i++){
      ...
      if (Math.hypot(p.x-c.x, p.y-c.y) >= R) { tLo = t; break; }
    }
    if (tLo === null) return;
  ...
  function drawTeardropLoop(p, above, color, width=3){
    ...
    // Arrowhead at end, direction from second-to-last point
    // Use derivative approximation near end
    const pPrev = {
      x: (end.x + ctrl.x)/2,
      y: (end.y + ctrl.y)/2
    };
  ...
  function maskedWords(chainArr, revealedSets){
    return chainArr.map((w, wi) => {
      return [...w].map((ch, li) => revealedSets[wi].has(li) ? ch : "_").join("");
    });
  }
  ...
  function drawChainHeaderAndSwatches(chainArr){
    ...
    // store word rects for swatches
    const rects = [];
    for (let i=0;i<wordsToDraw.length;i++){
      const w = wordsToDraw[i];
      ctx.fillStyle = COLORS.black;
      ctx.fillText(w, x, y);
      rects.push({x, yTop: y-24, w: widths[i], h: 28});
      x += widths[i];
    ...
    const swY = y + 14;
    rects.forEach((r, i) => {
      const c = WORD_EDGE_COLORS[i % WORD_EDGE_COLORS.length];
      const swX = r.x + r.w/2 - swW/2;
  ...
  function buildLegend(hiddenMap){
    ...
    const rowH = 32, pad = 12;
    const twoCols = nItems > 8;
    const colGap = 20, colW = 90;
  ...
  function draw() {
    ...
    const headerBottom = drawChainHeaderAndSwatches(chain);
    ...
    // hover node (hover highlight, regardless of dragging)
    hoverNode = hitNodeAt(pointer.x, pointer.y);

    // legend
    const legendBox = buildLegend(hidden);

    // draw edges with highlight logic
    for (let i=0;i<edges.length;i++){
      ...
      const alpha = (hoverNode == null)
        ? 0.42
        : ((e.a===hoverNode || e.b===hoverNode) ? 0.90 : 0.25);
    ...
    // determine which letters are currently revealed
    const revealedLetters = new Set();
    if (showWords){
      for (const [ch] of freq.entries()) revealedLetters.add(ch);
    } else {
      for (let wi=0; wi<chain.length; wi++){
        for (const li of revealed[wi]){
          revealedLetters.add(chain[wi][li]);
        }
      }
    }
    ...
    for (const ch of order){
      ...
      // show letter in node only if revealed
    ...
    // draw legend box
    drawRoundedRect(legendBox.legendX, legendBox.legendY, legendBox.boxW, legendBox.boxH, 10,
      COLORS.legendBg, COLORS.legendBorder, 2);
  ...
  canvas.addEventListener("pointerdown", (e) => {
    ...
    if (howtoOverlay.style.display === "flex") return;
  ...
  canvas.addEventListener("pointerup", (e) => {
    updatePointer(e);
    if (!dragging || !draggedLetter || !chain) { dragging=false; draggedLetter=null; return; }
    ...
    if (node && !showWords && !solved){
      if (node === draggedLetter){
        ...
        // wrong — but do NOT count error if node's letter is already revealed anywhere
